...
class VideoPlayer {
    ...
    _onSegment(data) {
        ...
        let wordArrayA = CryptoJS.lib.WordArray.create(data.buffer)
        let string  = wordArrayA.toString(CryptoJS.enc.Base64)
        var verified = verify.verify(string, data.encryptedSignature, CryptoJS.SHA256);

       
        //compare created hash with hash from payload
        if (verified == true){
            if (this._sourceBuffer.buffered.length) {
            ...
            if (this._sourceBuffer.updating) {
                this._lastSegment = data.buffer;
            } else {
                delete this._lastSegment;
                this._sourceBuffer.appendBuffer(data.buffer);
            }

         }
         else{
             //don't do shit with the incoming data because it is altered
         }
    }

...
(function () {
    ...
    var Bits, CRLF_CRLF, DAY_NAMES, DEBUG_DISABLE_UDP_TRANSPORT, DEBUG_HTTP_TUNNEL, DEBUG_OUTGOING_PACKET_DATA, DEBUG_OUTGOING_RTCP, DEBUG_RTSP, DEBUG_RTSP_HEADERS_ONLY, DEFAULT_SERVER_NAME, ENABLE_START_PLAYING_FROM_KEYFRAME, MONTH_NAMES, RTSPClient, RTSPServer, SINGLE_NAL_UNIT_MAX_SIZE, Sequent, TAG, TIMESTAMP_ROUNDOFF, aac, api, avstreams, config, crypto, dgram, enabledFeatures, generateNewSessionID, generateRandom32, h264, http, logger, net, os, pad, resetStreamParams, rtp, sdp, url, zeropad, record;

    var retryCount = new Map();
    var failedVerifies = new Map();
    var totalPacketsVerified = new Map();
    var totalReceivedPackets = new Map();
    ...
    streamSender = require('../service/streamSender');

    streamer = require('../service/streamers');

    rtp = require('./rtp');

    sdp = require('./sdp');

    h264 = require('../video/h264');

    aac = require('../audio/aac');
    ...
    RTSPServer = class RTSPServer {
        ...
        start(opts, callback) {
			...
			c.on('close', () => {
				...
				var leavingStreamer = streamer.getStreamers();

                        if(Object.entries(leavingStreamer).length > 0){
				let user = leavingStreamer[client.id];

				if(typeof user !== 'undefined'){
					if (user.phone !== undefined) {
						//log in a fancy color
						console.log('\x1b[36m%s\x1b[0m', 'User: ' + user.phone + ' total packets received: ' + totalReceivedPackets.get(user.phone))
						totalReceivedPackets.set(user.phone, 0);
						console.log('\x1b[36m%s\x1b[0m', 'User: ' + user.phone + ' failed to verify packet(s): ' + failedVerifies.get(user.phone));
						failedVerifies.set(user.phone, 0);
						console.log('\x1b[36m%s\x1b[0m', 'User: ' + user.phone + ' packet retry treshold violated:' + retryCount.get(user.phone)) 
						retryCount.set(user.phone, 0);
					}
				}
                        }

                        streamer.removeStreamer(client.id);
                        var recorder = record.getRecorder(client.id)

                        if (typeof recorder !== 'undefined') {
                            recorder.stopRecording();
                            recorder = null;
                            record.removeRecorder(client.id);
                        }
            ...
            if (client.uploadingStream != null) {
                ...
                switch (interleavedData.channel) {
                    case stream.rtspUploadingClient.uploadingChannels.videoData:
                        return this.onUploadVideoData(stream, interleavedData.data, senderInfo, interleavedData);
                    ...
                    case stream.rtspUploadingClient.uploadingChannels.audioData:
                        return this.onUploadAudioData(stream, interleavedData.data, senderInfo, interleavedData);
        ...
        respondTeardown(socket, req, callback) {
            ...
            // var streamId = RTSPServer.getStreamIdFromUri(req.uri);
            // console.log("uri: " + req.uri)
            // console.log("streamid: " + streamId)
            // var recorder = record.getRecorder(streamId)
            // console.log("stop recording")
            // console.log(recorder)
            // recorder.stopRecording();
            // recorder = null;
            // record.removeRecorder(streamId);
        ...
        respondRecord(socket, req, callback) {
            ...
            streamer.createStreamer(client.id, streamId, () => {
                ...
                let parameters = streamSender.getParameters(streamId);

                streamSender.sendStream(parameters, client.id, function (endedStream, streamName, reason) {
                    if (endedStream = true) {
                        console.log(reason + " | " + streamName);
                    }
                })
            });
            ...
            record.createRecorder(req.uri, client.id, streamId);
            var recorder = record.getRecorder(client.id)
        ...
        packetIntegrityCheck(stream, msg, rinfo, interleavedData, deepCopy){
            var packet;
            //      logger.warn "no client is uploading video data to stream #{stream.id}"
		packet = rtp.parsePacket(msg);
		
		let receivedPacketCount = totalReceivedPackets.get(stream.id.split('/')[1]);

            if(typeof receivedPacketCount === 'number'){
		    totalReceivedPackets.set(stream.id.split('/')[1], receivedPacketCount+=1);
            }
            else{
                totalReceivedPackets.set(stream.id.split('/')[1], 1);
            }

            var integrityPipeline = require('../service/integrityConnection').AllintegrityFrames;

            if (integrityPipeline !== 'undefined' && integrityPipeline.length > 0) {
                //console.log(integrityPipeline.length);
                for (const item of integrityPipeline) {
                    if (item.sequence === packet.rtpHeader.sequenceNumber && item.id == stream.id.split('/')[1]) {

                        let publicKey = a.length > 0 ? a[item.index].publicKey : 'invalid'
                        let verifier = crypto.createVerify('RSA-SHA256')
                        verifier.update(interleavedData.data)
                        let result = verifier.verify(publicKey, item.signature, 'base64')

                        if (result) {
                            integrityPipeline.splice(integrityPipeline.indexOf(item.signature), 1);
                            
			    let totalPacketsVerifiedCount = totalPacketsVerified.get(stream.id.split('/')[1]);

			    if(typeof totalPacketsVerifiedCount === 'number'){
				totalPacketsVerified.set(stream.id.split('/')[1], totalPacketsVerifiedCount+=1);
			    }
			    else{
				totalPacketsVerified.set(stream.id.split('/')[1], 1);
			    }

                            // if (integrityPipeline.length >= 15) {
                            //     integrityPipeline.shift();
                            // }
                            
                        }
                        else {

				let failedVerifiesCount = failedVerifies.get(stream.id.split('/')[1]);

				if(typeof failedVerifiesCount === 'number'){
					failedVerifies.set(stream.id.split('/')[1], failedVerifiesCount+=1);
				}
				else{
					failedVerifies.set(stream.id.split('/')[1], 1);
				}
                            
                        }
                    }
                    else {//packet not found

                        if (typeof deepCopy === 'undefined') {
                            //create deepcopy once
                            deepCopy = deepClone({
                                streamS: stream,
                                msgS: msg,
                                rinfoS: rinfo,
                                interleavedDataS: interleavedData,
                                retryCount: 1
                            });
                        }
                        ...
                        if (deepCopy.retryCount < 5) {
                            //retry after x seconds
                            setTimeout(() => {
                                deepCopy.retryCount += 1;
                                this.packetIntegrityCheck(deepCopy.streamS, deepCopy.msgS, deepCopy.rinfoS, deepCopy.interleavedDataS, deepCopy);
                            }, retryDelay)
                        }
                        else {
                            //Set retry count for user
                            let count = retryCount.get(stream.id.split('/')[1]);
                            retryCount.set(stream.id.split('/')[1], count ? 0 : count++);
                        }
                    }
                }
            }
                ...
                if (deepCopy.retryCount < 500) {
                    ...
                    //Set retry treshold reached count for user

		    let retriedPacketTresholdReachedCount = retryCount.get(stream.id.split('/')[1]);

		    if(typeof retriedPacketTresholdReachedCount === 'number'){
			    retryCount.set(stream.id.split('/')[1], retriedPacketTresholdReachedCount+=1);
		    }
		    else{
			    retryCount.set(stream.id.split('/')[1], 1);
		    }
                }
            }
        }
        ...
        onUploadVideoData(stream, msg, rinfo, interleavedData) {
            var packet;
            ...
            //check modulo 5 packets with integrity packets
            if (packet.rtpHeader.sequenceNumber % 5 == 0) {
               
                this.packetIntegrityCheck(stream, msg, rinfo, interleavedData)
            }

            //pass packets to stream
        ...
        onUploadAudioData(stream, msg, rinfo, interleavedData) {
            var packet;
            ...
            //check modulo 5 packets for integrity
            if (packet.rtpHeader.sequenceNumber % 5 == 0) {
            ...
            // console.log(retryCount.get(stream.id.split('/')[1]));
        }
